...
namespace DataAccess.Example
{
	 class Program
	{
		...
		const string mysqlConnectionString = "server=localhost;port=3306;database=sqltesting;user=root;password=@Canelo67";
		...
		private static ComapnyCreationResult CreateCompany(string companyName, string companyLocation, string adminEmail, string employeeEmail)
		{
			ComapnyCreationResult result = null;
			MySqlConnector.MySqlConnection connection = new MySqlConnector.MySqlConnection(mysqlConnectionString);
			connection.Open();
			...
			try
			{
				...
				result = new ComapnyCreationResult
				{
